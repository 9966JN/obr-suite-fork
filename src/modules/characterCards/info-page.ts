...
async function showCard(cardId: string, roomId: string) {
  ...
  try {
    const res = await fetch(
      `${location.origin}/characters/${encodeURIComponent(roomId)}/${encodeURIComponent(cardId)}/data.json`
    );
...
function render(d: any, cardId: string, roomId: string) {
  ...
  const rawUrl = `${location.origin}/characters/${encodeURIComponent(roomId)}/${encodeURIComponent(cardId)}/`;
